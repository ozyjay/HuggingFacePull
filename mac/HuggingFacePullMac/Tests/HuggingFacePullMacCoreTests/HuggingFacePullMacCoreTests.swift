...
struct HuggingFacePullMacCoreTests {
    ...
    func decodesQueueState() throws {
        let json = """
        {
          ...
          "hf_hub_cache": "/Users/example/.cache/huggingface/hub",
        ...
        #expect(state.hfHubCache == "/Users/example/.cache/huggingface/hub")
        #expect(state.items.first?.repoID == "Qwen/Qwen3")
        #expect(state.items.first?.xetEnabled == true)
    ...
    func apiClientRequests() async throws {
        ...
        MockURLProtocol.handler = { request in
            ...
            case ("POST", "/api/queue", _):
                ...
                #expect(text.contains("\"xet_enabled\":true"))
                return .json("""
                {"id":"1","repo_id":"Qwen/Qwen3","revision":"main","repo_type":"model","allow_patterns":["*.json"],"ignore_patterns":[],"xet_enabled":true,"deduplicated":false,"status":"waiting","error":null,"messages":[],"progress":{"phase":"waiting","overall":{"downloaded":0,"total":null,"percent":null},"current_file":null},"created_at":1,"updated_at":1}
                """)
            ...
            case ("POST", "/api/cleanup/scan", _):
                ...
                """)
            case ("POST", "/api/installed/delete", _):
                return .json("""
                {"ok":true,"freed_size":2048}
                """)
            default:
