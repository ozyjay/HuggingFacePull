...
struct SearchView: View {
    ...
    @State private var xetEnabled = false
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                ForEach(model.searchResults) { result in
                    SearchResultRow(result: result) {
                        ...
                        focusedField = .repoID
                        model.notice = Notice(
                            message: "Selected \(result.repoID). Choose download options, then add it to the queue.",
                            isError: false
                        )
                    }
                    Divider()
                ...
                Grid(alignment: .leading, horizontalSpacing: 12, verticalSpacing: 12) {
                    ...
                    GridRow {
                        Text("Transfer")
                        Toggle("Use Xet transfer for this download", isOn: $xetEnabled)
                    }
                }
...
struct QueueRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                Text(item.revision)
                Text(item.xetEnabled ? "Xet" : "Plain HTTP")
            ...
            if let item {
                HStack {
                    VStack(alignment: .leading) {
                        ...
                        Text("\(item.revision) | \(item.repoType.rawValue) | \(item.xetEnabled ? "Xet" : "Plain HTTP")")
                            .foregroundStyle(.secondary)
                    }
                    Spacer()
                    StatusBadge(status: item.status)
...
struct DetailGrid: View {
    ...
    var body: some View {
        Grid(alignment: .leading, horizontalSpacing: 16, verticalSpacing: 8) {
            ...
            detail("Transfer", item.xetEnabled ? "Xet" : "Plain HTTP")
...
struct InstalledView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            SectionHeader(title: "Installed snapshots", detail: "\(model.installedSnapshots.count) snapshots")
            List(model.installedSnapshots) { installed in
                HStack {
                    VStack(alignment: .leading, spacing: 4) {
                    ...
                    Spacer()
                    if model.hasInstalledRecord(installed) {
                        Button {
        ...
        .confirmationDialog(
            "Permanently delete this cached snapshot?",
            isPresented: Binding(
            ...
            Button("Delete \(installed.repoID) from Disk", role: .destructive) {
                pendingDeletion = nil
                Task { await model.deleteInstalled(installed) }
            }
        } message: { installed in
            Text("Unshared cached files for \(installed.repoID)@\(installed.revision) will be removed. This cannot be undone.")
        }
    }
}
