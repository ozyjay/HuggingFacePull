...
final class AppViewModel: ObservableObject {
    ...
    var installedSnapshots: [InstalledModel] {
        var seen = Set<String>()
        return ((state?.installedModels ?? []) + (state?.cachedModels ?? [])).filter {
            seen.insert($0.id).inserted
        }
    }

    func hasInstalledRecord(_ model: InstalledModel) -> Bool {
        state?.installedModels.contains(where: { $0.id == model.id }) == true
    }
    ...
    func queue(repoID: String, revision: String, repoType: RepoType, allowPatterns: String, ignorePatterns: String, xetEnabled: Bool) async {
    ...
    func removeInstalled(_ model: InstalledModel) async {
        do {
            ...
            notice = Notice(message: "Removed \(model.repoID) from the installed list", isError: false)
            await refresh()
        } catch {
            show(error)
        }
    ...
    func deleteInstalled(_ model: InstalledModel) async {
        do {
            let result = try await apiClient.deleteInstalled(InstalledRemoveRequest(
                repoID: model.repoID,
                revision: model.revision,
                repoType: model.repoType
            ))
            notice = Notice(
                message: "Deleted \(model.repoID) from disk; freed \(ByteFormat.string(fromByteCount: result.freedSize))",
                isError: false
            )
            await refresh()
        } catch {
