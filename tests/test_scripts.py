...
def test_powershell_workflow_scripts_exist():
    ...
    assert (scripts / "run-mac.ps1").is_file()
...
def test_powershell_scripts_cover_core_workflows():
    ...
    run_mac = (ROOT / "scripts" / "run-mac.ps1").read_text(encoding="utf-8")
    ...
    assert "mac/HuggingFacePullMac" in run_mac
    assert "Set-Location" not in run_mac
    assert '"swift" "build" "--package-path"' in run_mac
    assert "--show-bin-path" in run_mac
    assert 'Join-Path $binPath "HuggingFacePullMac"' in run_mac
    assert "nohup" in run_mac
    assert "Start-Process" not in run_mac
    assert "System.Diagnostics" not in run_mac
    assert "Invoke-Checked -FilePath $executable" not in run_mac
